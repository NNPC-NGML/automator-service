...
class AutomatorTaskControllerTest extends TestCase
{
    ...
    public function test_assign_a_task_from_head_of_unit_to_subordinate(): void
    {
        ...
        $this->actingAsAuthenticatedTestUser();
        $response = $this->postJson('/api/assign-task-to-user/', ["task_id" => $task->id, "user_id" => $user->id]);
        $response->assertOk()->assertJsonStructure([
            "status",
            "message"
        ]);
        ...
        // test job was dispatched by mocking the job
        foreach (config('nnpcreusable.AUTOMATOR_TASK_CREATED') as $queue) {
            Queue::assertPushedOn($queue, AutomatorTaskCreated::class);
        }
    }
}
